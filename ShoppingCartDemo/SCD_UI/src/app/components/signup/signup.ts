...
export class Signup  implements OnInit {
  ...
  constructor(private router: Router, private fb: FormBuilder, private service: UserService) {
    this.signupForm = new FormGroup({
      ...
      password: new FormControl<string>('12qw!@QW', { nonNullable: true, validators: [Validators.required, Validators.pattern('^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[#?!@$%^&*-]).{8,}$')] }),
      cpassword: new FormControl<string>('12qw!@QW', { nonNullable: true, validators: Validators.required }),
